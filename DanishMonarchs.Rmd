...
kings1 <- read.csv("kings.csv", na = "NULL")

kings2 <- read_csv("kings.csv", na = "NULL")

kings3 <- read.csv2("kings.csv", na = "NULL")

kings4 <- read_csv2( "kings.csv", na = "NULL")
```
Answer: 
...
```{r}
# YOUR CODE
duration_vector <- nykings$duration
average_duration <- mean(as.numeric(duration_vector), na.rm = TRUE)
print(average_duration)
library(dplyr)
average_duration_tidy <- nykings %>%
  summarize(average_duration = mean(as.numeric(duration), na.rm = TRUE))
print(average_duration_tidy)
```
...
```{r}
# YOUR CODE
average_reign <- mean(nykings$duration, na.rm = TRUE)
filtered_nykings <- nykings %>%
  filter(duration > average_reign)

print(filtered_nykings)

filtered_count <- filtered_nykings %>% count()
...
longest_ruling_monarchs <- nykings %>%
  arrange(desc(duration)) %>%
  slice(1:3)
longest_ruling_monarchs <- longest_ruling_monarchs %>%
  mutate(Days = duration * 365 + floor(duration / 4))
print(longest_ruling_monarchs)
```
...
ggplot(nynykings, aes(x = Mid_year, y = duration)) +
  geom_point() +
